    ...
    VkResult on_vkCreateDevice(gfxstream::base::BumpPool* pool,
        ...
        // has to infer what it could simply read.
        {
            ...
            const bool supported = std::any_of(
                devExts.begin(), devExts.end(), [](const VkExtensionProperties& e) {
                    return strcmp(e.extensionName, VK_EXT_METAL_OBJECTS_EXTENSION_NAME) == 0;
                });
            const bool alreadyOn =
                std::any_of(updatedDeviceExtensions.begin(), updatedDeviceExtensions.end(),
                            [](const char* n) {
                                return strcmp(n, VK_EXT_METAL_OBJECTS_EXTENSION_NAME) == 0;
                            });
