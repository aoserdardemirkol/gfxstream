    ...
    if (out->mExternallyBacked) {
...
#if defined(__APPLE__)
        // Back the guest render image with the same MTLTexture as the host ColorBuffer.
        ...
        if (importedColorBufferInfo.vimaMetalImported) {
            ...
            if (vimaGuestProviderTex) {
                ...
                GFXSTREAM_ERROR(
                    "CB %u: Metal texture provider returned no texture for format %s",
...
#if defined(__APPLE__)
        if (vimaGuestProviderTex) vk_insert_struct(createImageCi, vimaImportTex);
#endif
